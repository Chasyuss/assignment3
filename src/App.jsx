...
function App() {
  ...
  const filteredStudents = students.filter(student => minAge <= student.age);
  ...
  return (
    <div>
      ...
      <input type="number" value={minAge} onChange={(e) => setMinAge(Number(e.target.value))} /> 살 이상
      <ul>
        {/* TODO: map을 사용하여 필터링된 학생들의 정보를 표시하세요. 동명이인은 없다고 가정합니다. ex) 홍길동 - Age: 24, Grade: A */}
        {filteredStudents.map(student => (
          <li key={student.name}> {student.name} - Age : {student.age} , Grade: {student.grade} </li>
        ))}
      </ul>
    </div>
  );
